...
export default function LoginPage() {
  ...
  useEffect(() => {
    console.log('[Login Page] Auth state:', { isAuthenticated, user, accessToken: !!accessToken })
    if (isAuthenticated) {
      console.log('[Login Page] Redirecting to home...')
      router.push(`/${locale}`)
    }
  }, [isAuthenticated, user, accessToken, router, locale])

  const handleGoogleLogin = () => {
    // FRONTEND_API.md Section 1.1: Google OAuth 로그인 시작
